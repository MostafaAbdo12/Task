...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 z-[100] bg-black/40 backdrop-blur-md lg:hidden transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      ></div>
      ...
      `}>
        ...
        <div className="flex-1 px-6 overflow-y-auto no-scrollbar space-y-10">
          <div>
            <p className="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">البروتوكولات</p>
          ...
          <div>
            <div className="flex items-center justify-between px-4 mb-4">
              ...
              <button onClick={() => { onViewChange('categories'); onClose(); }} className="text-blue-400 hover:scale-125 transition-transform">
        ...
        <div className="p-6 bg-white/5 mt-auto">
          <div className="flex items-center gap-4 p-4 rounded-3xl bg-white/5 border border-white/5 transition-all group cursor-pointer hover:bg-white/10">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-nebula-purple to-nebula-pink text-white flex items-center justify-center font-black text-xl shadow-lg">
              {user.username.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0 text-right">
              <p className="text-sm font-black truncate">{user.username}</p>
              <p className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest">متصل بالشبكة</p>
            </div>
            <button onClick={onLogout} className="p-2 text-slate-500 hover:text-rose-500 transition-colors mr-auto">
...
const NavItem = ({ active, onClick, icon, label, count }: any) => (
  <button 
    onClick={onClick}
    className={`
      w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-500
      ${active 
  ...
  >
    ...
    {count !== undefined && (
      <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${active ? 'bg-nebula-purple text-white shadow-glow' : 'bg-white/5 text-slate-500'}`}>{count}</span>
    )}
  </button>
);
